...
namespace AntConc
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void btnTab4Convert_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var myFile = (File.ReadAllText(myOpenedFile)).Split();
                ...
                if (saveFileDialog1.ShowDialog() == true)
                {
                    if ((myStream = saveFileDialog1.OpenFile()) != null)
                    {
                        ...
                        worksheet.Cell(2, 1).Value = "Qidirilayotgan so'z : " + txtTab4Search.Text;
                        worksheet.Cell(2, 1).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);

                        worksheet.Cell(3, 1).Value = "Klaster turlarining umumiy soni : " + labNumberHits4.Text;
                        worksheet.Cell(3, 1).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);

                        worksheet.Cell(4, 1).Value = "Min. so'z: " + myUpDown1.Value + " Max. so'z: " + myUpDown2.Value;
                        ...
                        worksheet.Range(worksheet.Cell(2, 1), worksheet.Cell(2, 2)).Merge();
                        worksheet.Range(worksheet.Cell(3, 1), worksheet.Cell(3, 2)).Merge();
                        worksheet.Range(worksheet.Cell(4, 1), worksheet.Cell(4, 2)).Merge();
                        worksheet.Cell(5, 1).Value = "Darajasi";
                        worksheet.Cell(5, 2).Value = "Xillar";

                        int kk = 6;
                        foreach (ConcordanceHits rv in myTable)
                        {
                            worksheet.Cell(kk, 1).Value = rv.Id;
                            worksheet.Cell(kk, 2).Value = rv.myWords;
                            kk++;
                        }
        ...
        private void btnTab1Convert_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var myFile = (File.ReadAllText(myOpenedFile)).Split();
                ...
                List<WordTypes> conword = new List<WordTypes>();
                foreach (var t in indexOfFind)
                {
                    string S = "";
                    if (t >= 7)
                    ...
                    conword.Add(new WordTypes { Word = S });
                }
                ...
                if (saveFileDialog1.ShowDialog() == true)
                {
                    if ((myStream = saveFileDialog1.OpenFile()) != null)
                    {
                        ...
                        worksheet.Cell(2, 1).Value = "Qidirilayotgan so'z : " + txtTab1Search.Text;
                        worksheet.Cell(2, 1).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);

                        worksheet.Cell(3, 1).Value = "Klaster turlarining umumiy soni : " + labNumberHits.Text;
                        worksheet.Cell(3, 1).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
                        ...
                        int kk = 4;
                        foreach (WordTypes rv in conword)
                        {
                            worksheet.Cell(kk, 1).Value = rv.Word;
                            kk++;
                        }
